...
try {
	...
	$arrDadosRelatorio = $objFachadaBDR->relatorioChamadosPorAssunto($arrParametro);
}
catch (Exception $ex) {
...
<?php
	...
	foreach ($arrDadosRelatorio as $arrDados) {
		if ($intQtdeLinhas < Config::getQtdeMaxBarrasGrafico()) $arrChartData[] = "['". $arrDados["descricao_assunto"] ."', ". $arrDados["fechados_dentro_prazo"] .", ". $arrDados["abertos_dentro_prazo"] .", ". $arrDados["fechados_fora_prazo"] .", ". $arrDados["abertos_fora_prazo"] .", ". $arrDados["cancelados"] ."]";
		else {
	...
	if (!empty($arrOutros["qtde"])) {
		$arrChartData[] = "['Demais Assuntos', ". $arrOutros["fechados_dentro_prazo"] .", ". $arrOutros["abertos_dentro_prazo"] .", ". $arrOutros["fechados_fora_prazo"] .", ". $arrOutros["abertos_fora_prazo"] .", ". $arrOutros["cancelados"] ."]";
	}
?>
...
<?php echo implode(", ", $arrChartData); ?>
	...
	var options = {
		title: 'Chamados Por Assunto - <?php echo Util::formatarMes($arrParametro["intMes"]) ?> / <?php echo $arrParametro["intAno"]; ?>',
		isStacked: true,
		titleTextStyle: {
			color: 'black',
			fontName: 'Verdana',
