...
try {
	...
	$arrDadosRelatorio = $objFachadaBDR->relatorioChamadosPorGrupo($arrParametro);
}
catch (Exception $ex) {
...
<?php
	...
	foreach ($arrDadosRelatorio as $arrDados) {
		if ($intQtdeLinhas < Config::getQtdeMaxBarrasGrafico()) $arrChartData[] = "['". $arrDados["descricao_grupo"] ."', ". $arrDados["fechados_dentro_prazo"] .", ". $arrDados["abertos_dentro_prazo"] .", ". $arrDados["fechados_fora_prazo"] .", ". $arrDados["abertos_fora_prazo"] .", ". $arrDados["cancelados"] ."]";
		else {
...
<?php echo implode(", ", $arrChartData); ?>
	...
	var options = {
		title: 'Chamados Por Grupo - <?php echo Util::formatarMes($arrParametro["intMes"]) ?> / <?php echo $arrParametro["intAno"]; ?>',
		isStacked: true,
		titleTextStyle: {
			color: 'black',
			fontName: 'Verdana',
